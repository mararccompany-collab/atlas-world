...
function simulateWar(attacker: Country, defender: Country): { rounds: RoundResult[]; winnerId: string; winnerAr: string; reason: string } {
  ...
  const airDetail = aAir > dAir ? ('???? ??? ???? ?????? ?????? ' + attacker.nameAr)
    : dAir > aAir ? ('????? ???? ????? ?????? ?????? ' + defender.nameAr)
    : '????? ??? ??? ???????';
  rounds.push({ roundName: 'Air War', roundNameAr: '????? ??????', winnerId: airWinner, winnerAr: airWinner === attacker.id ? attacker.nameAr : defender.nameAr, detail: airDetail, attackerScore: Math.round(aAir), defenderScore: Math.round(dAir) });

  const aNavy = getNavyScore(attacker);
  const dNavy = getNavyScore(defender);
  const navyWinner = aNavy > dNavy ? attacker.id : defender.id;
  const navyDetail = aNavy > dNavy ? ('????? ' + attacker.nameAr + ' ?????? ????? ??? ??????')
    : dNavy > aNavy ? ('??????? ' + defender.nameAr + ' ???? ???????')
    : '?? ???? ???? ????? ????? ???????';
  rounds.push({ roundName: 'Naval War', roundNameAr: '????? ???????', winnerId: navyWinner, winnerAr: navyWinner === attacker.id ? attacker.nameAr : defender.nameAr, detail: navyDetail, attackerScore: Math.round(aNavy), defenderScore: Math.round(dNavy) });
  ...
  const groundDetail = aGround > dGround
    ? (attacker.nameAr + ' ????? ???? ????? ' + defender.nameAr + ' ????? ????? (' + losses.bLoss + ' ????)')
    : (defender.nameAr + ' ??? ?????? ????? ?????? ' + attacker.nameAr + ' ????? (' + losses.aLoss + ' ????)');
  rounds.push({ roundName: 'Ground War', roundNameAr: '????? ??????', winnerId: groundWinner, winnerAr: groundWinner === attacker.id ? attacker.nameAr : defender.nameAr, detail: groundDetail, attackerScore: Math.round(aGround), defenderScore: Math.round(dGround) });
  ...
  if (aFinal > dFinal) {
    reason = attacker.military.nuclear && !defender.military.nuclear
      ? ('???? ???????? ??????? ??????? ??????? ??????? ???? ' + attacker.nameAr + ' ????? ??? ' + defender.nameAr)
      : ('??? ????? ????? ?? ???? ?????? ?????? ????? ' + attacker.nameAr + ' ?? ??? ??????? ??? ' + defender.nameAr);
  } else {
    reason = defender.military.nuclear && !attacker.military.nuclear
      ? ('???? ????? ?????? ???????? ????????? ???? ' + defender.nameAr + ' ?????? ' + attacker.nameAr + ' ??? ????????')
      : ('???? ?????? ?????? ???????? ???????? ????????? ????? ' + defender.nameAr + ' ??? ' + attacker.nameAr);
  }
  rounds.push({ roundName: 'Final Assault', roundNameAr: '?????? ???????', winnerId: finalWinner, winnerAr: finalWinner === attacker.id ? attacker.nameAr : defender.nameAr, detail: reason, attackerScore: Math.round(aFinal), defenderScore: Math.round(dFinal) });
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white" dir="rtl">
      ...
      <header className="relative z-10 bg-gradient-to-b from-[#0a0a0f] via-[#0a0a0f]/95 to-transparent sticky top-0 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 py-6 flex items-center justify-between">
          <button onClick={onClose} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition-all">? ??????</button>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-red-300 to-orange-400 bg-clip-text text-transparent">?? ????? ??????</h1>
          <div></div>
        </div>
      </header>
        ...
        {war.status === 'idle' && (
          <div className="max-w-4xl mx-auto space-y-8">
            <div className="text-center">
              ...
              <h2 className="text-4xl font-black bg-gradient-to-r from-red-300 to-orange-300 bg-clip-text text-transparent">????? ??????</h2>
              <p className="text-gray-400 mt-4 text-lg">???? ?????? ??????? ??? ????? ??????</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <label className="block text-red-400 font-bold text-lg mb-3">?? ???????</label>
              ...
              <div>
                <label className="block text-blue-400 font-bold text-lg mb-3">?? ???????</label>
            ...
            {attacker && defender && (
              <div className="bg-gray-900/60 border border-gray-700/50 rounded-2xl p-6">
                <h3 className="text-xl font-bold text-center mb-6">?? {attacker.nameAr} ?? {defender.nameAr}</h3>
                <div className="grid grid-cols-2 gap-6">
                  <div className="text-center">
                    <div className="text-6xl mb-2">{attacker.flag}</div>
                    <div className="font-bold text-red-400">{attacker.nameAr}</div>
                    <div className="text-gray-400 text-sm mt-2">????? ????????: {getMilitaryScore(attacker).toFixed(0)}</div>
                    <div className="text-gray-500 text-xs">{attacker.military.personnel} ???? | ???? #{attacker.military.rank}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-6xl mb-2">{defender.flag}</div>
                    <div className="font-bold text-blue-400">{defender.nameAr}</div>
                    <div className="text-gray-400 text-sm mt-2">????? ????????: {getMilitaryScore(defender).toFixed(0)}</div>
                    <div className="text-gray-500 text-xs">{defender.military.personnel} ???? | ???? #{defender.military.rank}</div>
                  </div>
                </div>
              </div>
            )}
                ...
                <div className={'inline-block bg-gradient-to-br from-gray-900 to-gray-950 border-2 rounded-3xl p-8 shadow-2xl ' +
                  ...
                  <div className="mt-8 flex gap-4 justify-center">
                    <button onClick={() => { setWar({ status: 'idle', currentRound: 0, rounds: [], finalWinnerId: '', finalWinnerAr: '', reason: '' }); setShowWinner(false); }}
                      className="px-6 py-3 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 transition-all font-bold">?? ????? ????????</button>
                    <button onClick={() => {
